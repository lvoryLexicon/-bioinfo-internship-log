...
def plot_tracks_from_bigwig(bw_path,
    ...
    for i, (chrom, vals, chrom_length) in enumerate(tracks):
        ...
        # 绘制填充区域
        ax.fill_between(x, y_offset, y_offset + norm_vals, step='mid', alpha=0.9, color='royalblue')
        ...
        # 在右侧写染色体名字并用方框框起来
        chrom_label_x = max_bins + max_bins * 0.08  # 右侧位置
        # 添加方框
    ...
    ax.set_xlim(-max_bins*0.0, max_bins + max_bins*0.18)  # 给右侧标签留更多空间
